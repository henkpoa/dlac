...
| `/dl giftbox` | **Open every reward box in your inventory** — Goblin Giftboxes, the Goblin Gatherbox, the Tiny/Timeworn/Titanic Tackleboxes, the Mamool Ja/Lamia/Troll Troves, the alexandrite Purses, the Forgotten Pouches, the Moat/Forest Carp Creels and the Old Case — one at a time, in a fixed order. The gate is per box, not per run: a giftbox pays out up to 5 items so it needs 6 free slots, while a purse or a pouch pays 1 and needs only 2 — so a tight bag keeps opening what it can afford instead of stopping on the first box it cannot. It tells you how many are left and the smallest number of slots that would unblock it — it never opens into a bag that cannot hold the contents. It waits for each box's count to actually drop before using the next, so it paces itself to the server instead of firing on a timer; a box that does not open stops the run rather than being tried again. `/dl giftbox stop` halts it, `/dl giftbox status` counts without opening. Also answers to `/dl gb`, `/dl box` and `/dl boxes`. A tray icon offers the same run when you are carrying boxes *somewhere worth opening them* — standing at an Ephemeral Box as a Crystal Warrior, or in town as anyone else; the command itself works anywhere |
