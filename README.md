...
them to game events, toggle combat modes, and let situational gear resolve itself â€”
...
1. **Install** â€” drop the `dlac` folder into `Ashita\addons\`, then
   `/addon load dlac` (add that line to your Ashita boot script to load it every time).
2. **Open the GUI** â€” **CTRL+K** (or `/dl ui`).
3. Click the red **Setup** button (top-right). Works on *any* profile:
   - your existing profile â€” whether ffxi-lac, hand-written, or anything else â€” is
...
| **All Equipment** | Browse everything you own â€” or the full CatsEyeXI catalog â€” with search and stats |
| **Sets** | Build sets by hand or **Auto-build** from stat weights; level-scaling candidate lists per slot; live score |
| **Triggers** | Wire sets to the game: statuses, spells (by skill / type / element / `contains` / exact name), abilities, items, weaponskills â€” plus player-defined **Modes** with live toggle buttons |

### Triggers, in short

A trigger is *condition(s) â†’ set*. All matching triggers apply, most specific last, so
...
**live on the next action** â€” no reload. `/dl why` explains exactly what fired and why.

### Modes

Named switches that triggers can match. Two kinds:

- **Toggles** (e.g. `DT`) â€” on/off; overlay your damage-taken set over whatever else
  won. Highest priority: manual intent always wins.
- **Cycles** (e.g. `Weapon`: Melee â†’ Ranged â†’ Caster) â€” an ordered value list, exactly
...
**keybind** in the GUI â€” applied automatically at profile load, no OnLoad code.
...
- **Staff** â€” your best *usable* Iridescence option for that cast: HQ/NQ elemental
  staff vs a universal weapon (Chatoyant Staff, Foreshadow +1, Iridal Staff), highest
  tier wins, ties go to the universal.
- **Obi** â€” the matching elemental obi (or the universal Hachirin-no-obi) only when
...
fallback â€” being under-leveled never blocks the slot. Owned staves/obis are
...
| `/dl why` | Explain the last dispatch per handler â€” what matched, what equipped |
...
| `/dl prune` (then `/dl prune commit`) | Remove gear.lua entries for items you no longer own anywhere â€” dry-run first; checks every container incl. storage |
...
| `/dl t <where>` | Teleport earring: lock Ear2, equip the matching earring (norg, jeuno, sandy, bastok...), wait, use it â€” no arg lists destinations |

## Safety

- Setup never deletes your code â€” dispatch calls are *appended*; originals are backed
...
**Start at [docs/HANDOFF.md](docs/HANDOFF.md)** â€” environment, hard rules, and current
