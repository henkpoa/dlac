...
**Engage/target edge**:
The signal a **Job helper** acts on: one outgoing action packet the player's own client sent, decoded once by the central edge service (`feature/engagewatch`) and pushed to subscribers. Two kinds — the **engage edge** (you attacked something) and the **retarget edge** (your battle target changed while engaged, which is what auto-target rolling to the next mob sends). The entity is taken FROM the packet and carried with the edge, never re-read at consumption time: by then the target has moved on, and sending a pet at the wrong mob is precisely what capturing it prevents. A per-target debounce (5 seconds) means the same entity notifies at most once a window while a different one notifies immediately, so client re-sends and target stutter never reach a consumer. First consumer: the BST Helper's **Fight** switch, whose three ways are Off / **When I attack** (engage edges only) / **Follow my target** (both). Because a helper acts only on edges — never on a poll, never on a timer — *Heel is respected for free*: a pulled-back pet stays back until the player's next attack or target change.
_Avoid_: auto-engage (names the ACT, and reads as the addon fighting for you — the naming law; the player still attacks, dlac only forwards the pet), auto-target (the game's own feature, which is one SOURCE of a retarget edge, not the edge), tick / poll (there is neither)

**Pet vitals**:
The other signal a **Job helper** acts on: your pet's presence, HP%, TP and name, answered by the central service (`feature/petvitals`) and published to subscribers once per dispatch beat. Unlike an **Engage/target edge** it IS a poll, because "my pet is hurt" is a state and not an event. Two laws travel with it. **Dead pet = no pet** — a pet at 0 HP% is not a pet any consumer may act on, the same rule `gData.GetPet()` has always encoded. And **presence is two-state on purpose**: "no pet" and "the read could not be made" answer identically, because every consumer of the answer issues a command or spends an item, and a read we cannot make is not permission to do either. Individual vitals stay nil-able — a present pet whose HP could not be read is reported honestly, never guessed at. First consumer: the BST Helper's **Reward rule**, which asks for the **Action sequence** while the pet sits below the player's pet-HP% threshold, once per **retry lockout** window.
