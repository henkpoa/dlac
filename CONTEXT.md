...
**Gear rule**:
A gear helper assigned to a specific set PIECE through its Behaviour rules, as opposed to occupying a slot: the piece is worn only while the rule holds, otherwise the slot's next-best pick takes the slot. Two shapes on the entry wrapper. **Dual Wield** (`dw = true`, shipped 2026-08-04): the piece is a *candidate* only while the Dual Wield trait is actually up — a flatten-time gate like a mode gate (no tier bump; rank stays normal), judged by `utils.isDualWieldAvailable` (the server's own trait bit, `HasAbility(1554)`), with the trait bit itself a `rebuildSets` signal so a BLU trait-set swap or the 0x0AC packet racing a sub change re-flattens on its own. The Suppanomimi case: no trait, no point wearing it. **Type automations** (`autoType` + `removePrio`/`acc`): the engine decides at *equip time*, releasing candidates in **Removal Priority** order — foundation only on main; the first member, **AutoAcc**, lives on `feature/autoacc` pending GM approval.
...
**Vendored module**:
A **Job helper** whose source of truth is another repository and which CI copies in wholesale. Today there is one: `jobhelpers\blu\bludex\` (Blue Magic set planning), authored at github.com/henkpoa/bludex and pushed here by that repo's `sync-dlac.yml` on every push to *its* main, landing on dlac `dev` — main stays a human merge like anything else. The folder is **overwritten, never merged**, which is the whole of what the term has to convey: an edit made here survives until the next sync and then vanishes without a trace or a conflict. Every copy carries a `VENDORED.md` saying so and naming the bludex commit it came from, and that stamp is written only when a vendored file actually differs (since 2026-08-11 — before that it was rewritten every push, so dlac collected commits that changed nothing but the stamp). A Vendored module is a Job helper in every other respect: same folder shape, same one row on the **Job Helpers** tab, same one unit of server approval.
_Avoid_: submodule (it is a plain copy — there is nothing to init or update, and `git submodule` will not find it), fork, third-party plugin (same maintainer, first-party code — the parked third-party idea is the one under **Job helper**)
...
**Disfavour**:
CatsEyeXI's bad-luck protection on **lottery** NM pops, and the reason an NM's *base* chance is not its chance: each completed placeholder **round** — every one of that NM's placeholders killed once — lifts the roll until a pop is guaranteed (`chance = 100 / max((100/base) - rounds * (1 - base/100) / 2, 1)`; base 5% → 40 rounds, 10% → 20, 15% → 14, 20% → 10). It is **hand-carried** in `feature/nmlookup.lua`'s `M.DISFAVOUR` with its source and verification date, because it exists in no branch of the public server repository — it lives in the private `catseyexi` overlay, which is empty publicly — and scraping a wiki for it was rejected as a way to silently poison the odds. **The anchor tests are its source** (`NM40*`/`NM41*`), which is why the mid-curve is pinned and not only the endpoints. The per-NM base still comes from the generated table; only the curve is carried. Applies to `kind = "lottery"` and nothing else — a `scripted` / `timed` / `weather` / `night` NM has no curve at all, even when the table lists placeholders for it.
_Avoid_: bad luck protection / BLP (say Disfavour — it is the server's own name), pity timer, "X% per kill" (the flat reading this replaced), pop rate (that is the base alone)

**Pop count**:
The placeholder kills this character **personally witnessed** at one camp, counted passively and turned into **Disfavour** rounds so `/dl nm` can show the *current* chance instead of the base (`feature/nmtrack`, per character in `<char>\dlac\nmcounts.lua`). Automatic, with nothing to arm — there is no such thing as an accidental placeholder kill, and the failure mode of an opt-in tracker is losing the data the player sat down to collect. Counted **by target index, never by name**: Uleguerand holds ten Buffalo and only six (354-359) are Bonnacon's placeholders, so a name would credit kills that never rolled. Two observations already in the addon are joined to get there — the **Engage/target edge** knows the *index* of what you attacked, the death line knows *what* died — and one engagement can credit at most one kill, so the error left over is an under-count. Three properties make it worth showing at all. It is a **floor**: the server's counter is zone-wide and shared, so other players' kills raise it invisibly. It goes **stale** on any break in observation — zoning, a relog or a reload, or an age past the NM's own repop ceiling — and a stale count shows its raw number and when it was last vouched for but **renders no percentage**, because the error a break makes runs optimistic. And kills that **cannot roll** (inside the post-kill cooldown, or while the NM is up) are tallied as wasted effort instead of being sold back as progress. Evidence of a pop — seeing the NM alive or seeing it die — resets it; staleness is otherwise sticky, and `/dl nm <name> reset` is the one manual door.
_Avoid_: pop tracker / kill tracker (nothing is tracked in the world — it is a count of what you saw), pop timer (nothing is timed; the repop window is the table's), "the server's count" (it is deliberately NOT that — see floor), spawn counter

**Drop table**:
What an NM gives, as `/dl nm <name>` reads it out (`feature/nmloot`, from generated `data/nmdrops.lua` — 973 pools, 5372 rows, crawled from the **live API** and keyed by **server pool id**, which each NM's `pool` field joins to). Never the clone's `mob_droplist`: the public repo's module loader mounts a `catseyexi` overlay that is empty there, so live differs in both item and rate (Mee Deggi gives Ochiudo's Kote at 10%; the clone says Ochimusha Kote at 5%). A **roll** is the unit, not an item: each ungrouped row rolls on its own, a **group** rolls once at its group rate and then exactly ONE item inside it wins **by weight** — so a grouped row's `r` is a weight and rendering it as a percentage would misstate the 2009 grouped rows, more than a third of the table. Duplicate rows are two independent rolls and are never deduped (Leaping Lizzy lists one item at 240 *and* at 150). **Steal and Despoil are not drops at all** and get their own section, because a kill will never give them. Rates carry the server's own eight **tiers** (1000 always, 240 very common, 150 common, 100 uncommon, 50 rare, 10 very rare, 5 super rare, 1 ultra rare); a rate outside those eight is still a valid weight and shows its percentage with **no tier name** rather than being snapped to the nearest one. Item names are absent from the table on purpose and resolve from the **client's own resources**, lazily and one id at a time, so they match what the client calls them and never run before login; an id that will not resolve renders **as an id**.
_Avoid_: loot table / drop list (say Drop table), drop chance for a grouped row's `r` (it is a **weight**; the chance is its **share** of the group), "Steal drop" (Steal is not a drop), inventing a tier name for an off-tier rate

**TH verdict**:
Per drop, whether Treasure Hunter is worth bringing for it — the answer `/dl nm <name>` gives beside the **Drop table** (`feature/nmloot`, issue #154). It is answerable because **TH is a bracket lookup, not a multiplier**: the rate selects a rarity **bracket** (the eight **tiers** ×10, `always` dropped off the top) and the TH level selects the value inside it, so the rate at any level 0-14 is arithmetic — the server's own table, ported verbatim, working out of 10000 on a rate handed in ×10 (stored 150 → 1500 in → 4500 at TH4 = **45%**). Four rules travel with it: an **ungrouped** row takes TH on its own rate; a **grouped** row takes it on the **group rate** only; TH **never** touches which member of a group wins, so the shares are unmoved and still sum the same; and a rate already at 100% **short-circuits unchanged**, which is the verdict worth printing — Ochiudo's Kote is a 10% share of a group that always drops, so no amount of TH moves it, and *saying that in words* is what saves a player a gear set where an unchanged number would not. Because the rate only *selects* a bracket, an **off-tier rate does not read back as itself** (the 68 rows at `gr = 750` read 24% at TH0, 64% at TH4) — the line says the stated rate only picks the bracket rather than letting that read as TH lowering a rate. Figures are opt-in (`/dl nm <name> th4`); the verdict line is not.
_Avoid_: TH multiplier / "TH×" (it is a lookup — a multiplier is the mental model this replaces), TH bonus, applying TH to a group member's weight (it applies to the group rate; the weight is not a rate), calling an unchanged number a verdict (the words are the verdict)

**NM Compendium**:
The in-game reference for field NMs — the **Floating window** half of what `/dl nm` answers in chat (`ui/nmui`, PRD #151, shell + list in #156, the card and the third mode in #157). One master-detail window: the filter and the result list on the left, the **NM card** on the right. Its defining property is that the searches are **filter modes over ONE list**, not separate windows: all three — by name, by area, and by **drop** — return **NMs**, so one list serves all of them and clicking a zone *or a drop* merely **re-filters the list you are already looking at**. That is the whole difference from the Chocobo dig search, whose two searches return different *kinds* of thing (items and zones) and therefore need a second window for a cross-link to land in. The window **owns no answers**: matching, the row list, the zone roster and each row's note all come from `feature/nmlookup`, the same module the chat command asks — so a weak match is labelled a **guess** and gibberish is **refused** on both surfaces, from one implementation and one `GUESS_FLOOR`, and on a score tie the entry **with placeholders** wins on both. Opened from `/dl nm window [name]` (bare, it opens on the zone you are standing in); drawn from exactly one site, gearui's `d3d_present`, like every other floating window here.
_Avoid_: NM browser / NM window (say NM Compendium — it is the feature's name), NM search (it is a browser with filter modes; "search" names one of them), dig-search-style two windows (the shape this deliberately is not)

**NM card**:
The right pane of the **NM Compendium** — everything the feature knows about one NM in one place, and the reason the window is worth opening (`ui/nmui`, issue #157). Four blocks: the **placeholders** (their name, their count and the target **indexes** to watch, with the **FilterScan** filter for exactly those spawn points appliable in one click); **how it pops** (the pop kind, the repop window, the base chance stated as a floor, the **Disfavour** curve and the rounds to a guaranteed pop); **your count** (the **Pop count**'s live state — rounds witnessed, the current chance, or the cooldown / primed / **stale** states); and the **Drop table**, each drop carrying its **TH verdict** at a level chosen on the card. It **computes nothing**: every figure is consumed from `feature/nmlookup`, `feature/nmtrack` and `feature/nmloot`, which is what keeps the window and the chat command from disagreeing about the same monster — and it is why the stale-count rule holds *by construction*, since `nmtrack` answers `chance = nil` for a stale record and there is simply no number to render. An NM with no placeholders still gets a card: the block says there are none and the one below states how it pops instead. Every drop on it is a click target that pivots the list to the by-drop mode, the mirror of clicking a zone.
_Avoid_: detail pane / detail view (say NM card), NM summary, recomputing anything on it (the card renders; the three feature modules answer)

**By-drop lookup**:
The **NM Compendium**'s third filter mode: type an item and the list becomes every NM that drops it (`feature/nmlookup.dropRows` over `feature/nmloot`'s reverse index, issue #157). It is the **Drop table**'s join run backwards — item → pools → the NM entries carrying them — and the index is built once, because the drop table cannot change while the client runs. Item **names** are not in that index: they resolve from the client's own resources like every other drop name, so the list is retried while any id is still unresolved and frozen the moment they all answer (a pre-login read must never latch — ADR 0007). Matching here is **strict**, unlike the by-name mode: only a real name hit counts (`ITEM_FLOOR`), because across 2183 items an edit-distance guess is not a hedge but noise — so a name it does not recognise is **refused** rather than answered with a list of near-misses. A row is therefore **never a guess**: the table says that NM drops that item, and only the item name was ever matched.
_Avoid_: item search (it answers with NMs, not items), reverse drop table (say by-drop lookup), treating a low item score as a guess about the monster
