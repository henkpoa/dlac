            ...
            if not MP_HOLD_EXEMPT[lslot] then
                ...
                if nm ~= nil then
                    ...
                    if low[lslot] == nil or mp < low[lslot] then
                        low[lslot], lowRf[lslot] = mp, rfv;
                    elseif mp == low[lslot] and rfv < (lowRf[lslot] or 0) then
                        -- ties take the LEAST refresh: assuming the incoming
                        -- piece lacks it keeps a refresh battery deep (safe).
                        lowRf[lslot] = rfv;
                    end
                end
            end
...
if inLac() then
    ...
    ashita.events.register('d3d_present', 'dlac-dispatch-tick', function()
        pcall(function()
            ...
            -- Feed the recovery-tick measurement (banded ladder, v88): every
            -- 0.4s reading of current MP; upward jumps are ticks, split by
            -- standing/resting. Cheap, always on -- the margins are already
            -- calibrated the moment maxmp gets enabled.
            if _mpb ~= nil then
                pcall(function()
                    local p = gData.GetPlayer();
