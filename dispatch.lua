            ...
            local function vLadderOf(src, slot)
                ...
                if cn2 ~= nil then
                    ...
                    if r2 ~= nil and r2.rladder ~= nil then
                        local ok2, lad2 = pcall(r2.rladder, slot, cState[cn2], ctx);
                        if ok2 then return lad2; end
                    end
                    return nil;
                end
                return M.candidatesFor(src, slot);
            end
            ctx.reserveSuppressed, ctx.reserveIneligible, ctx.reserveReplace =
                M.reserveResolve(entries, rslotOf, vLadderOf);
            ...
            if wslot ~= nil and LAC_SLOT_OK[wslot] then
                ...
                if srcName ~= nil then
                    ...
                    if lad ~= nil and type(lad.items) == 'table' and #lad.items > 0 then
                        local names = {};
                        for _, r in ipairs(lad.items) do names[#names + 1] = r.name; end
                        print(string.format('    ladder (%s): %s', tostring(srcName), table.concat(names, ' > ')));
                    end
                end
                return;
