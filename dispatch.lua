...
M.VERSION = 85;   -- 85: /dl plan applies the paired-slot veto (v83's rule) -- the plan advertised 'ear1: Loquac. Earring +20 gain' while the live engine vetoes that pick (single copy worn in ear2), so the overview promised an equip that never happens. The plan's pick now walks the ladder past pair-vetoed rungs exactly like the engine (falls to the next wearable rung, tags the row '[pair: X worn in ear2]'; a slot whose every rung is vetoed says so; dup-owned stays advertised). Tests MPS8*. Display only -- dispatch rules untouched.
