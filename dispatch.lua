...
-- Max-MP reconciliation (v86, pure -- tests MR*). Ashita's GetMPMax() can go
-- STALE across gear/job churn: field round 6 read 975/1052 while the client
-- bar said 975/975 -- the full-pool gate never opened (no equips) and
-- small-delta holds read as spent (early releases). The party MP PERCENT
-- rides the same packet family as current MP and pins the true max inside
-- [cur*100/(mpp+1), cur*100/mpp] (mpp is floored); mpp >= 100 pins it
-- EXACTLY (max = cur). A GetMPMax outside the window is stale and clamps to
-- the nearest edge; inside it, it is trusted as-is. cur/mpp unreadable (or
-- an empty pool) leave max untouched -- degrade to today's behavior.
function M.mpReconcileMax(cur, max, mpp)
    ...
    local lo = math.ceil(cur * 100 / (mpp + 1));
    local hi = math.floor(cur * 100 / mpp);
    if max == nil or max < lo then return lo; end
    if max > hi then return hi; end
    return max;
end
...
local function playerMP()
    ...
    -- anchor that catches a stale GetMPMax (see M.mpReconcileMax, v86).
    pcall(function() mpp = AshitaCore:GetMemoryManager():GetParty():GetMemberMPPercent(0); end);
    return tonumber(cur), M.mpReconcileMax(cur, max, mpp);
end
