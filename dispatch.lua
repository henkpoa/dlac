...
M.VERSION = 158;  -- 158: GEAR AVAILABILITY IS AN ARBITER REFUSAL (Henrik 2026-08-01: "I think arbiter needs to have this intelligence centrally, as well as report these things in arbiter monitor" -- and, on where it must NOT go: "It is FINE if claimants file ladders where some of the pieces are ineligible... which also saves us the trouble of adding yet another field they need to populate"). Field case: a Lv75 BRD parks a Minstrel's Coat in the Mog Safe and lowers his level until the Coat is his set's best-by-level Body. It wins the ladder, is written into the plan, and then dies SILENTLY in equipcore.planSet, whose locate pass first-fits over the EQUIP-ELIGIBLE bags only (gearoracle.equipBags: Inventory + the 8 Wardrobes) -- so the slot keeps whatever was worn and the rung BELOW the Coat is never asked, because by then the ladder is gone. The whole selection chain asked LEVEL and never asked the BAG (utils.slotLadder gates on item level/mode/sub-pairing, resolveVirtual's chain walks on usableAt(level), mpRungs on level+pair). That question is now asked ONCE, in gear\arbiter, where the ladders already are: availVerdict(floor, have) is a SECOND refusal reason riding the SAME fall loop the v135 dominance verdict rides -- refuse a rung, ask ladderOf for the next, re-judge, run to a fixed point. No claimant changed and no claimant gained a field: every row with an rladder (Craft, HELM, Fishing, Chocobo) and the floor's own candidatesFor inherit the fall, and a claimant added tomorrow inherits it without meeting it. haveEquippable is the impure half (bagCounts -- equip-eligible containers, lowered names, 1s cache), THREE-VALUED: only an explicit false refuses, and an unreadable OR EMPTY bag map answers nil, because a two-valued read would say "you own nothing" at char select / mid-zone / mid-load and strip all sixteen slots at exactly the moments dlac must sit still (ownedcache's own law: a failed lookup must never take a feature away). Apostrophe-tolerant on the miss path -- the client says "Arhat's Gi" and anything catalog-sourced says "Arhats Gi", and a spelling gap must never demote gear you are holding. An unavailable piece is hidden from the dominance view BEFORE it runs: it is not on your body, so it neither DEFENDS its slot nor RESERVES another one (the law reserveVerdict already states for a refused piece, applied to the other reason) -- a Coat rotting in the Safe no longer holds Head empty from a slot it can never occupy. A slot whose WHOLE ladder is refused is not killed: planSet cannot locate those names either, so it keeps what is worn (Henrik's ruling). reserveResolve gains a 4th return, purely for display: fall = { refused = {[slot] = {{name, why}, ...}}, dead = {[slot] = the piece the PLAYER authored, not the last rung reached} }. The Arbiter Monitor renders the WHOLE ladder with every refused rung struck through and its reason, and marks the cell without a hover ('*' = wearing its second choice, '!' = whole ladder refused); /dl why <slot> prints the same lines off the same record (one record, three renderers). Omitting `have` is byte-for-byte the old behaviour, which is what leaves every direct caller -- immediate equips, headless suites -- untouched. THE RECEIPT rides along: vLadderOf is the one door every ladder passes through -- floor sets one side, claimant rladders the other -- so it now notes each one down as it hands it over, and recordDecision keeps THAT instead of rebuilding the rung list afterwards from contest.src. Two bugs die with the rebuild: contest.src is written only by the trigger floor, so a Craft/HELM/Fishing/Chocobo fall had no rungs to show at ALL; and the rebuild re-asked LATER, so even the floor's own list could answer differently (a bag moved, the level changed) than the list the decision was actually made from -- the ladder that decided is now the ladder you read, the same law the trace already runs on. Free by construction: only ladders the fall ALREADY fetched are recorded (the arbiter asks for one only when it must refuse something, and that laziness is why a quiet dispatch costs nothing), and slots that never needed one keep the derivation as their fallback. The Monitor's 'set:' line stays quiet when the ladder names a different source -- the floor writes contest.src for every slot it touches, winner or not, so a claimant-won slot used to print a trigger set above a fall that happened nowhere near it. Tests UA1-UA8, DR9-DR12. 157: MODE KEYBINDS MOVE TO THE REGISTRY (feature\keybinds, Henrik 2026-07-30: "if a bind exists, it needs to be able to tell where and with what and block them from binding over it"). The trigger loader no longer issues /bind itself: it collects this job's mode binds and hands the whole group to keybinds.syncGroup('mode:', ...), sorted (pairs over modeDefs has no order and two modes contending for one key must resolve the same way twice). Three behaviours change and all three were bugs: a key another feature holds is REFUSED and its holder named instead of silently stolen; the PREVIOUS job's mode binds are RELEASED on the next load (nothing here ever unbound one, so they outlived their job for the session); and an unchanged bind is still left completely alone, which is the /bind-storm guard this loader grew in the field. _boundKeys survives as the DEGRADED path for a copy that cannot reach the registry (the seeded twin, headless), byte-identical to the old behaviour. No trigger, condition or equip path is touched. Tests KB1-KB40. 156: dayMatch trigger condition (Henrik, field: "there are items that give you bonus solely if the day match what you're casting") -- weatherMatch's sibling, the THIRD environment test: true when the CURRENT day's element equals the action's element. dayMatchesAction reads gData.GetEnvironment().DayElement (LAC data.lua / nativedata WEEK_DAY_ELEMENT -- the same field netForElement already scores), cached once per dispatch on ctx.del beside ctx.wel. DISTINCT from BOTH neighbours: dayWeatherBonus is the obi's signed net over day AND weather WITH opposition (on a matching day in opposing weather it reads 0 and stays quiet while a day-only item is paying out), weatherMatch has no day term at all. Unlike weather there is no "clear day" -- all eight weekdays carry an element -- so only an unreadable read is unknown. Precast+Midcast, tier 30, true/false polarity; unknown day or no action element matches NEITHER (never fires blind). ADR 0029. Tests DM1-DM24. 155: THE JOBHELPER ROW + ACTION SEQUENCE (issue #138, PRD #135). A new CLAIMANTS row, 'JobHelper', rides the standing rank walk for every Job helper's Action sequence -- but it is NOT in ARB_ORDER_DEFAULT: its Claim Priority position is remembered PER JOB, so jobHelperPlace weaves it into the live order (and /dl prio) directly below its per-job anchor (default Locks -- feature\jobhelpers.placedOrder owns the placement + store), and the row hides with zero modules installed. Its active/claim/status read feature\actionseq (the singleton sequencer: claim, verify worn through the oracle, fire once, release -- next arbitration restores); apply rides equipResolved at its rank like any claimant. New engine seams for the sequencer's live glue: M.wornName (public worn-name read), M.isLockedSlot, M.kickDefault. The sig leg is APPENDED (CLAIMANT_SIG_ORDER), so the nine existing legs stay byte-identical and no live session retraces until a sequence claims. claimantLabel renders 'JobHelper' as 'Job helper'. Tests CR* (registry pins updated -- JobHelper the one per-job extra), AS*/RC*/PF*/JHR*/JHW*. 154: THE ACTION FEED (Integration surface, the anchor's signal). Every non-Default dispatch -- including the no-trigger-matched bail -- appends one stub to a 32-deep ring (M.getActions): aseq, os.clock (fine-grained where the 1-second _trace stamp collides two fast casts), the ctx snapshot with the join key, and decSeq linking the stub to the decision it produced when it produced one. recordDecision now returns that seq. The observer turns decSeq-less stubs into `dispatch` ANCHOR envelopes -- "seen, decided nothing, here was your TP" -- so a consumer's join never reasons from silence; a stub WITH a decSeq is covered by its worn envelope (one anchor per action, never both). Engine behavior unchanged; Default deliberately never stubs (its changes speak through the decision ring). Tests IN*. 153: THE STREAM'S ENGINE SEAMS (Integration surface, step 2). Three small additive cuts, no equip behavior change: (1) worldWatch's absence bookkeeping runs even UNARMED and M.worldAbsentOutlasted(now) reads it -- the stream's lifetime gate is the SAME law/constant/timestamp as the holds' watch (ZONES SURVIVE, v146), read-only, and a job change deliberately does not show in it (a job change is data to a stream consumer, not a reason to stop); tests WW1-3. (2) ctxSnapshot captures THE JOIN KEY when an action carries one -- actionId (the 0x01A decode the engine already did), actionCategory, targetIndex -- so a ring record can be joined by integer against the consumer's own 0x028 traffic (design 5.1). (3) '/dl stream' routes to feature\integration (routing only; the observer is addon-side -- ADR 0014 holds: the engine does not export). Tests IN1-IN9 drive the observer itself. 152: THE DECISION RING (the Arbiter Monitor's record; ADR 0027 follow-on, 2026-07-28 -- ONE record, three renderers). Every dispatch whose OUTCOME moved appends one record to a 50-deep session ring (M.getDecisions): the plan snapshot as display names (taken BEFORE the one send retires ctx.planOut -- the plan is the only correct source at decision time), the stashed structured contest, the source ladders AS THEY WERE ASKED (candidatesFor at append time -- a pinned old record must never re-derive), and a defensive ctx snapshot (job/levels, HPP/MPP/TP, status/moving, day/weather/moon, modes, buffs, pet). The law is Henrik's 'only push changes': the fingerprint is resolved items PLUS each slot's winning claimant (M.decisionFp, pure) -- idle churn appends nothing, an action resolving to what is already planned appends nothing, and a dragged rank row that flips a winner under identical items DOES append, because the rank order is a retrace-sig leg now (the |ao leg; without it /dl why kept stale attribution after a drag). Empty plans record nothing; ladders/ctx build only on append, so the frame path pays one fingerprint + compare. Tests DR1-DR8. 151: THE MAXMP FOLD (ADR 0027, stage 6 -- the last ratified stage; Henrik's item-3 ruling: 'the Arbiter is the aware one'). The woven per-slot MP branch and the mp-stage post-pass are DELETED; MaxMP is a registry row like every other claimant. Its claim is the band target map (mpClaimFor via the row's `claim`); its apply filters the claim through the four gates -- remove-respect (v91), movement yield (v96), sticky pairs (v93/v94, museum #7), RSlot eligibility (v78) -- against the SAME-DISPATCH VIEW (ctx.planOut for rows already applied + the strongest unapplied above-rank claim per slot, museum #9's lag-free signal), then dresses via equipResolved at its rank. Ceding is APPLY ORDER now: rows above apply later and overwrite, Locks strip via the ordinary respect('MaxMP'), the Disabled ceiling filters at the write seam -- ctx.mpCeded and ctx.mpRespectLocks are RETIRED, the weave's last ctx-threads. What stays in equipResolved is the one true CONSTRAINT, mp-hold (POST_ORDER head): a worn battery in a slot the bands do not target holds against an MP-lighter incoming piece, unless the asking claimant (`who`, equipResolved's new 4th arg) ranks at or above MaxMP. Supporting moves: the rank order HOISTS above the claim build pass (ctx.rankOf exists before any builder runs -- ranks exist before anyone claims), mpBands' lock consult reads ctx.rankOf directly (a locked slot's rungs still leave the threshold math, so the bands cannot drift from what can dress), and mpBands memoizes on ctx so ONE dispatch samples ONE moment (the purity ruling). The bands (mpbands.lua) and the resolver family (mpRungs/mpBestPick/mpStickyPairs/mpStageEligible) are UNTOUCHED -- WHEN and WHAT unchanged, only the DELIVERY moved. Tests: CR3/CR3b inverted (no registry exceptions left), ARE* re-drives ceding as apply order + the sibling-duplicate sight, LV6/LV7 drive the row's respect, new MH* pins the mp-hold constraint, PL2 pins the new POST_ORDER. 150: ONE PLAN, ONE SEND (ADR 0027, stage 3's last slice -- the diagnosis's section 2.5 closed). During a dispatch every equipResolved pass MERGES its resolved table into ctx.planOut -- last-writer-wins, the same order the equipengine buffer used to accumulate N separate sends in, so the final map is identical by construction -- and the dispatch sends ONCE after the apply walk. The equipengine finally sees the whole intent as one set: a better satisfied-check, fewer packets, and the buffer stops being the hidden merger; 'the final plan' exists as a VALUE for the first time. A held slot (nil'd in its pass) is simply absent and keeps its keep-worn meaning; the MP weave is untouched (it still resolves per-table exactly as before -- only where the result GOES changed); direct callers (immediate equips, tests) carry no collector and send immediately as always. Test OS1 pins exactly-one-send with the whole plan; NK26 pins the map unchanged. 149: THE PAIR LAW MOVES HOME + HELM's LADDER (ADR 0027, two small steps, zero behavior change). The Range/Ammo pair family -- splitPair, SKILL_ARCHERY, pairsWith (the three-valued law), trinketRangeDrop and trinketWornDisplace, ADR 0010's decision rules verbatim -- MOVED to gear\arbiter.lua with the rest of the decision core; dispatch re-exports every name (resolveAmmoPlan, the trinket-vs-ranged post-pass and the TB* tests keep their doors). And the HELM registry row gains its rladder (the chain in collect mode, mirroring helmOverlayFor's inner ctx with hs.gather as the category) -- the claim-side ladder roster is complete: Craft, HELM, Fishing, Chocobo. 148: THE BELTS COME OFF (ADR 0027, step-1 cleanup). ctx.pinReserved, pinReservedSlots and nakedVoidsPinReserve are DELETED WHOLE -- mechanisms #3 and #9 of the review's nine-item pairwise inventory, closed. The v43 flap they guarded (set equips Head, pinned Tunic reserves it, server strips, forever) is the cross-rank verdict's one general rule since v141: a dominant pin reserver's claim enters the merged floor at the Pins row and SUPPRESSES the slots it reserves (ARK4), pins-vs-naked resolves by ROW strength (pins above Naked keep slots and reservations, below lose them -- no void dance), and the belt rode as a redundant second copy through v141-v147's field rounds before coming off. The trinket worn-displace guard drops its pin term (a pin's Ammo is safe by overlay order -- its own pass writes after the displace); the per-slot chain sheds the pin-reserved branch. Tests AL34-41 and NK15-17 retired with the mechanisms they pinned; the general rule's pins are ARK4 + the live suite. 147: CLAIM-SIDE LADDERS (ADR 0027 stage 4, last slice -- future-proofing by design: no craft/hobby item carries an RSlot today, so nothing changes in the field until one exists; the day it does, a beaten CLAIM piece falls down its own resolver chain instead of dying v135-style). resolveVirtual's chain families (craft/HELM/fish/choco) gain an `all` collector -- the SAME walk and gates in collect mode, one code path, never a twin; chainLadder() shapes the result; Craft/Fishing/Chocobo registry rows carry `rladder` (Craft mirrors craftOverlayFor's overrides; HELM's waits on its gather-override state field, one documented line when needed). Claim entries whose row has an rladder get a '\031'-prefixed src (a control char no user set name can carry); the verdict's ladderOf routes it to the row, everything else to candidatesFor. Tests ARK14-ARK14e. 146: TWO LOCK-LIFETIME RULINGS, both Henrik's, both pre-promotion (2026-07-27 night). (1) ZONES SURVIVE: "I want /dl locks and /dl naked to persist through zones." The world read goes 0/nil during a ZONE LOAD exactly as at character select, so M.worldWatch no longer drops on absence alone -- only on OUTLASTING a zone (WORLD_GONE_S = 60s; sitting at character select crosses it, a load never plausibly does) or on a live job change; the relog-to-another-job case stays caught because the tick's _tickJob latch advances only on LIVE reads, so a return from absence is judged against the job you LEFT with. A same-job relog that beats the window now survives -- the price of zone persistence, the kinder failure. Applies to all four holds on the one watch: naked, locked set, slot locks, free equip (NK28/LS14/DS13 re-pinned with injectable `now`). (2) A LOCKED SLOT ENTERS THE VERDICT AS THE PIECE IT FROZE: his locked-cloak case -- locking Body with the Royal Cloak worn instantly emptied Body, because v145 stripped the cloak's dead floor claim and Idle's Head piece landed with nobody reserving Head, so the server displaced the very piece the lock froze. The lock's verdict entry is now the WORN NAME at the Locks row (not a '(locked)' placeholder): it defends its slot AND its reservations count as a dominant reserver, one entry covering both directions; an empty locked slot still contributes nothing (the v144 waiver, subsumed). ARK11/ARK13*. 145: A LOCK-VETOED CLAIM IS NO CLAIM AT ALL (Henrik's locked-empty-Head field case, minutes after v144: locked EMPTY Head + worn Royal Cloak + Movement claiming Genbu's Kabuto -- the verdict ruled the cloak ineligible against a hat the lock then vetoed at the equip chain, so Body fell to Scorpion Harness +1 for a claim that never landed; the verdict and the veto disagreed about whether the claim was real). The law: a claim from a row that RESPECTS locks can never land in a locked slot, so it must not count in the verdict either -- floor entries and respect-locks claim rows strip locked slots (copy, never mutating the live sets/claims tables; zero cost with no locks set) before the merge. Punch-through rows (above Locks: Pins, Naked) keep their locked-slot claims, because those WILL land. Composed with v144: locked EMPTY Head -> the dead Movement claim vanishes, nothing contests, the cloak is undisturbed and Head stays exactly as frozen; locked WORN hat -> the '(locked)' sentinel still defends and the cloak falls. Tests ARK12-ARK12e. 144: the EMPTY-LOCK WAIVER (Henrik's field refinement of v142, minutes after testing it: "if we can make it still equip royal body if it's empty, it would be nice"). A lock on an EMPTY slot no longer defends in the verdict -- freezing "empty" is exactly what a reservation preserves, so the reserver may land and the locked slot stays exactly as frozen (his case: a locked, empty Head no longer stops the Royal Cloak). A lock on a WORN piece defends as v142 shipped it: the reserver landing would make the server strip the very piece the lock froze. DELIBERATE ASYMMETRY: the free-equip ceiling's defense stays unconditional -- free equip promises "hand-equip any time and it STAYS", and an empty free slot must keep that promise for the future hand-equip (a reserver sitting next door would have the server strip whatever the player later puts there, with dlac looking innocent). The waiver lives at the verdict-entry build site (the worn read is engine state); ARK11/ARK11b pin the gate and the asymmetry as source. 143: /dl why <slot> -- THE CONTEST DRILL-DOWN (ADR 0027 item 4, ratified: depth on demand). The bare /dl why keeps its one-screen budget untouched; the new slot argument prints the FULL contest for that one slot from the trace's stashed structured contest (the same explain that attributed + the verdict's word + the floor ladder), never a re-derivation: every claimant in rank order with the winner marked, then the verdict's line when it had one -- 'fell: Royal Cloak -> Scorpion Harness +1 (it reserves Head -- owned above)' / 'INELIGIBLE (no rung to fall to)' / 'held EMPTY: reserved by X' -- then the source set's ladder rungs via candidatesFor ('ladder (IdleSet): Royal Cloak > Scorpion Harness +1'). The contest is stashed at retrace (verdict maps captured before they retire) and carried across quiet dispatches exactly like the rendered lines. An unknown slot word says so and lists the vocabulary. Tests WY1-WY1c pin the stash. 142: SENTINEL DEFENSE ROWS (ADR 0027 stage 4, slice 2 -- the ratified free consequence lands). The lock veto and the Disabled ceiling join the cross-rank verdict as DEFENDERS: '(locked)' / '(free equip)' placeholder names at their rank rows -- never equipped (they exist only in the verdict merge), never reserving (no manifest entry, mask 0), but STANDING, so a reserving piece whose target slot is locked or free-equip is beaten by rank and falls down its ladder (or dies v135-style) instead of landing and letting the server empty a slot the player explicitly froze or took back. Appended inside the same rank walk as the claims, so a claim ABOVE Locks overwrites the placeholder in the merge -- the punch-through law untouched (ARK9). Keys canonicalized into the merge (the floor is raw-key; lac-case 'head' next to canonical 'Head' would split the slot in two). Tests ARK8-ARK10b. 141: DOMINANCE ACROSS RANK (ADR 0027 item 2 + stage 4, first slice -- the ratified cross-rank law goes live). The OVERLYING EYE's verdict now merges every matching rule's set AND every built claim, each entry carrying its Arbiter ROW: strength = rank outright across rows (the slot contest's own ordering, asked about the reserved slots), trigger priority within the floor, ties favor the reserver, ord excluded (a trigger-file reorder can never flip a reservation). WORN PIECES ARE NOT CLAIMS (the Mindie ruling generalized): while the dispatch verdict is the one reservation authority (ctx.reserveGlobal), NO pass -- floor or claim -- runs the single-set + worn fallback; a worn reserver nothing re-claims defends nothing and the server displaces it when the winner lands. Direct callers (immediate equips, tests) keep the old judgement untouched. The stage-2 FALL rides across rank: an ineligible floor piece falls down its source ladder even when a CLAIM beat it -- Henrik's craft-bench acceptance (2026-07-27): the Royal Cloak no longer drops the Craft claim's Midras's Helm +1; the helm lands, and if Idle re-claims Body the cloak falls to Scorpion Harness +1. A dominant claim-row reserver now suppresses floor slots by the same one rule (pins' bespoke ctx.pinReserved stays as a redundant belt this slice; its retirement + sentinel rows in the verdict (lock/ceiling bulldozing) + claim-side ladders are the next slices). Tests ARK1-ARK7b. 140: THE ARBITER MODULE (ADR 0027, stage 3, first slice -- zero behavior change by construction). The pure decision core is EXTRACTED to gear\arbiter.lua, equipcore's sibling under the same layering law (no AshitaCore, no gData, no io -- ARM1 loads it with no stubs at all): the slot + rank vocabulary (LAC_SLOTS/LAC_SLOTS_CANON/RSLOT_ORDER/hasBit/rslotText, ARB_ORDER_DEFAULT + the pinned rows + arbOrder's restore-at-default law, the LOCK_HELD sentinel + arbLockClaim), the reservation family (reservedDrops; v135's reserveFloor/reserveVerdict; stage 2's reserveResolve), and the resolve/explain family (arbResolve/arbCededAbove/arbExplain/arbWhyLines) -- all moved VERBATIM, with every old M.* seam re-exported by direct assignment (rawequal-pinned, ARM2*: a delegation, never a twin; LOCK_HELD keeps its identity). NEW: ARB.arbitrate(session) -- one arbitration per dispatch, first slice: it owns the APPLY ORDER (the reverse rank walk over active claims) and M.dispatch executes the plan instead of walking inline. Later slices grow the plan into per-slot contests + the trace /dl why renders; stage 4 hands it the ladders and cross-rank dominance. dispatch hard-requires the module (failing loud beats deciding wrong); the harnesses seed package.loaded (run_tests) or resolve via the searcher hook (smoke_ui). Tests ARM1-ARM3b; the whole AR*/LV*/AKD*/AKF* families now drive the module THROUGH the old doors. 139: THE FALL (ADR 0027, stage 2 -- the deferred half of the v135 ruling, "go for the next available piece"). An INELIGIBLE piece no longer strands its slot: M.reserveResolve (reserveFloor -> reserveVerdict -> the ladder walk, a fixed point capped at three verdict re-runs) asks the source set's ladder (M.candidatesFor, stage 1) for the next rung, each replacement re-judged by the same verdict -- a rung that reserves a slot a higher claim owns is refused too, and a rung that reserves DOMINATED slots suppresses them exactly like any dominant reserver. The refused PIECE falls; a reserved SLOT still never does (the asymmetry law, ADR 0027). The floor entries carry their source set name through reserveFloor (src) so the fall knows whose ladder to ask; inline `equip` rules have no ladder and keep the unwritten-slot behavior; a ladder that runs dry reads INELIGIBLE in /dl why exactly as v135 did; the winner lands via ctx.reserveReplace in the refused piece's own writer pass ('Body=Royal Cloak fell -> Scorpion Harness +1 (reserves Head -- owned above)'). Field case this exists for (Henrik, Mindie BRD, 2026-07-27): Idle Body = Royal Cloak > Scorpion Harness +1 under Movement Head = Genbu's Kabuto -- the Kabuto wins Head, the cloak is ineligible, and Body now falls to the harness instead of standing bare. Tests AKF1-AKF10. 138: LADDERS ON DEMAND (ADR 0027, stage 1 -- docs/design/two-way-arbiter.md section 7; zero behavior change by construction). utils.BuildDynamicSets no longer truncates: ONE evaluator (utils.slotLadder) produces each slot's ordered candidate list -- the field-proven pick comparator (active-mode tier, live-range tier, item level, earlier-on-ties) kept as a SORT -- and the flatten derives its pick from the ladder's head (utils.flattenHead owns the 'marker|fallback' and AutoAcc composition, the ONE site). M.candidatesFor(setName, slot) is the on-demand door: reads the authored Dynamic store (the ladders never left memory), answers with the SAME context the last flatten used (utils._lastFlattenCtx), memoized per utils._laddersRev (bumped each rebuild). The old two-pass walk's virtual re-adoption quirk is PRESERVED and pinned (LD8): with no eligible mode item, the last BARE virtual beats a later mode-gated one. No consumer yet -- stage 2 (an ineligible reserver falls to its next rung) is the first. Tests LD1-LD10. 137: the AutoAmmo /dl prio line is JOB-AWARE (Henrik's stage-0 field report, 2026-07-27: "auto ammo is ON ... when it really isn't on" -- the line read ammoStateOn, which answers for the FILE: ANY job's enabled section made it ON, while the job being played had no setup and the claim never built). M._ammoJobLine answers for the CURRENT job -- fmt-2 section enabled + non-empty, or fmt-1 with resolveAmmoPlan's jobs-gate mirrored -- and an off line NAMES the jobs that do have it set up ("off (this job -- set up on DRK)"), so both directions of the confusion answer themselves; nil job keeps the file-level fallback. ammoStateOn itself is UNTOUCHED: it feeds the dispatch bail, where "some job claims" is the right question. Tests CR10*. 136: THE CLAIMANT REGISTRY (ADR 0027, stage 0 -- docs/design/two-way-arbiter.md section 7; zero behavior change by construction). One CLAIMANTS table, one row per rank row except the Triggers floor, and M.dispatch's six hand-copied per-claimant shapes now ITERATE it: the ensure/active pass, both bail guards, the claims map, the retrace-signature legs (CLAIMANT_SIG_ORDER pins the old byte order, so a live session does not retrace once on upgrade), the rank-walk applies and /dl prio's status lines. The 2026-07-25 architecture review measured adding a claimant at 15 hunks across 6 non-adjacent regions, three of which failed SILENTLY when forgotten (a missed bail term was a claimant that never dispatched; a missed signature leg was a stale /dl why); a new claimant is now ONE row + one ARB_ORDER_DEFAULT entry. MaxMP stays the one inline-built claim (order-coupled to mpRespectLocks/mpCeded; its equip stays woven until ADR 0027 stage 6) and Disabled/MaxMP stay out of both bails by design (free equip or a bare mode is not a reason to dispatch). Tests CR* pin the registry's shape as data; NK26 drives the rewired body end to end. 135: MULTI-SLOT DOMINANCE (Henrik's ruling 2026-07-27, two field cases the same afternoon). A piece that reserves other slots is a CANDIDATE only while the claim wanting it is dominant over every slot it takes -- dominant, it wins its slot AND claims the reserved ones (left EMPTY; the server clears them itself); beaten, it is INELIGIBLE and its own slot goes unwritten. The gap was structural: the overlay applies each matching rule's set through its OWN equipResolved, so reservedDrops never saw more than one rule at a time and priority never got a vote. Hunklor SAM -- Movement(25) Body=Kupo Suit (reserves Legs) over Idle(20) Legs=Amir Dirs: Idle wrote the legs, Movement wrote the suit, the server stripped the legs, repeat every ~0.4s with moving=true throughout. Mindie SCH -- Idle(20) Body=Royal Cloak (reserves Head) under Movement(25) Head: reservedDrops' `worn` arm made it WORSE, the cloak already on his back reserving Head out from under the rule that OUTRANKED it ("This is the wrong logic"). Both are the one rule pulling opposite ways. M.reserveFloor merges every matching rule's set in APPLY order (last-writer-wins, each slot tagged with the priority that won it) and M.reserveVerdict judges it in RSLOT_ORDER -- dominance resolved BEFORE anything is suppressed, so a piece never reserves on its way out, and a claimed slot cannot itself claim (Body takes Legs, so the Legs piece cannot go on to take Feet). The engine builds the floor BEFORE the first write and retires it right after the trigger loop: Claim layers keep the single-set + worn judgement they were field-tested with, because this floor describes the TRIGGER contest ("on a trigger level") and a rank contest is not a priority number. NOT YET: "go for the next available piece" -- utils.BuildDynamicSets collapses each slot's list to ONE name before the engine sees it (the AutoAmmo rung-2 trap), so an ineligible piece leaves its slot unwritten rather than falling to the next rung; carrying per-slot alternates is the follow-up. Tests AKD1-26. 134: AutoAmmo's ladder learns the LEVEL (field report 2026-07-27, Henrik's DRK: a list of Acid Bolt 15 / Blind Bolt 10 / Crossbow Bolt 1, sorted best-first by the panel's own button, loaded nothing at level 8 and nothing at 10 -- "when I make the slot empty, it still tries to auto equip acid bolts"). resolveAmmoPlan asked three questions per entry (flag, pair since v128, stock) and never asked the fourth, so the top entry won whatever the player's level was, and the overlay COLLAPSES A LADDER TO ONE NAME before the equip layer sees it -- there is no rung 2 to fall to, unlike a set slot, which is why the failure was total and silent. Now a fourth gate (M.resolveAmmoPlan's `wearable`, mirroring gear\equipcore checkUsable: level < Level, Jobs & 2^job) reads f.gate -> itemGate(id), the live client resource for an item already counted in the bags, with the entry's stored level as fallback; UNKNOWN NEVER DISQUALIFIES (the pairsWith three-valued law). The level is playerLevel(ctx) -- so `/dl set level main N` wins, exactly as it does in the set flatten and the virtual slot entries; reading MainJobSync straight would have left AutoAmmo the last picker in dlac that ignores the override. The Default arm now RE-JUDGES what is worn (empty slot, or the worn ammo is over-level, or it is one of OURS and no longer the best rung) instead of only reloading an empty slot -- the other half of the report, "I had acid bolts on me already but didn't change" -- while anything worn that is NOT on the list stays untouchable (a Midshot set's trinket must survive every idle tick; owning the slot outright is ADR 0010's flap through a third door). Default also holds while ctx.syncHold is up, since the level is now an input; an OVERRIDE never arms that hold and must not. resolveAmmoPlan returns four values (plan, why, code, chat): stock-outs print edge-triggered on a change of cause, level-driven rung changes print nothing at all (Henrik). docs/design/auto-ammo.md Section 10; tests AM51-AM68. 133: the purge, Phase 3-4 -- native-aware surfaces + the allowlist. /dl check reads the NATIVE home (stamp, sets file) and the seeded-copies/shim lines died (#131 CLOSED); debug.lua's handoff reads follow (handoffDir = profiles.dataDir) and requestEngine is a no-op; charDir has NO legacy fallback; '/dl profile migrate go' no longer reloads anything; every 'Reload LAC' user string became /dl reload or died; gearui equip-now uses the game's native /equip; legacy job files are READ-ONLY (the delete-static writer refuses); the module-local legacy path fallbacks (gearimport x2, gearexport, augments, debug) are gone. Tests PRG1-2 pin the allowlist: a uashitacast string literal may exist only in the keep-list readers. 132: the purge, Phase 2 -- LEGACY MODE DIES WHOLE. inLac(), the gProfile/gFunc world, the LAC-hosted engine path, readJobSets + the legacy sets fallback, warnShadowedStatics, the HandleEquipEvent wrap, the LAC tick half, the request-file bridge and both lockstyle engine halves are DELETED; nativeMode() is constant true, the engine flag is retired in place, /dl engine is status-only. One state, one engine, one sets store. 131: the LuaShitacast purge, Phase 1 (docs/design/lac-purge-plan.md). The engine SELF-SWAP died whole -- M.swapWanted, trySelfSwap, the __dlacEngineRoot handshake, tests SW* -- because the LAC seeder that fed it died in dlac.lua the same day: with seeds frozen forever, the watch could only ever DOWNGRADE a running engine to stale bytes. profiles.migrate step 4 (rewrite the job file as a clean shim) died too: originals now stay in place, inert and importable (the keep-list promise), and the shim WRITER (shimFileText/SHIM_BODY/BOOT_LINE) went with it -- only the recognizers (SHIM_MARKER, isCleanShim) remain for files already on disk. Setup writes NO job files anymore (MIGRATE_BOOT/STARTER_PROFILE deleted; storage + base sets + starter triggers ARE the setup). The LAC-alive polite ask (lacAlive/shouldAskUnloadLac) is gone; the coexistence tripwire remains the backstop. Nothing in dlac writes under config\addons\luashitacast\ anymore -- that tree is read-only import/migration territory. 130: the native flatten no longer waits for the GUI (Xvs field case 2026-07-27). Every utils lookup assumed "loaded first in the LAC state" -- the job shim's first require -- but the NATIVE state has no shim and NOTHING loaded utils at boot: package.loaded read nil, installSets skipped the flatten, and every install refused "flatten produced no sets (world not settled)" every 0.4s until a GUI picker's own lazy pcall(require, 'dlac\\utils') happened to run (gearui's isDualWieldAvailable / subCandidateOk -- the accidental medic; that's why it read as per-JOB in the field: the healed session's job "worked", a reload broke it). utilsModule() now requires lazily at call time -- cycle-safe, see its comment. Tests RQU0-RQU2. 129: /dl disable -- free equip, the native-era answer to /lac disable (Henrik 2026-07-26: "make it claim the slot / slots, then don't do anything at all with it, so people can free equip all they want without DLAC intervention"). Not a lock (a lock is a veto INSIDE the rank walk -- four rows punch through it) and not a claim to dress (every other row wins a slot in order to put something there; this one wins it to write nothing). It is the CEILING: pinned first in ARB_ORDER_DEFAULT, undraggable like the Triggers floor, and enforced at engineEquipSet -- the one write seam -- rather than in equipResolved's per-slot chain, because the whole-table post-passes that run after that chain write slots the set never named (mp-stage's battery, trinket-vs-ranged's worn-Ammo displace) and would put a slot straight back. Registered as a claim ONLY for attribution, so /dl why and the Priority panel can name it and woven MaxMP cedes it for free. Shares M.worldWatch with naked / locked set / slot locks (v124's one lifetime rule): drops on a main job change and on leaving the world, never written to disk. The Equipped tab's "Free equip" switch now drives THIS instead of /lac disable, which under the native engine talked to a LuaAshitacast that no longer does the equipping and therefore did nothing at all. ADR 0024; tests DS1-DS30.
...
function M.dispatch(event)
    ...
    pcall(function()
        ...
        local vFall, vAsked = ctx.reserveFall, ctx.reserveAsked;
        ctx.reserveSuppressed, ctx.reserveIneligible, ctx.reserveReplace = nil, nil, nil;
        ctx.reserveFall, ctx.reserveAsked = nil, nil;
