...
-- ...and it is the one door that STASHES the queue, so every reader of
-- M._modeLockQ (the claimant's signature leg, the contest the decision record
-- carries) sees the queue that belongs to the same computation as the plan.
-- A stash only `ensure` refreshed was a trap: any other caller read a queue from
-- an older dispatch and could not tell.
function M.modeLockLive()
    local plan, q = M.modeLockPlan(_trig.modeDefs, M.modes, M.modeSeq);
    M._modeLockQ = q;
    return plan, q;
end
...
local function resolveVirtual(marker, ctx, slot, all)
    ...
    if mk == 'dlac:autohelm' then
        ...
        if tonumber(a.fmtver) and tonumber(a.fmtver) < 16 then
            return nil, 'refresh gathering gear in the Gear Helpers tab';
        end
        if h.model == 'extra-rolls' then
            local categories = type(h.categories) == 'table' and h.categories or {};
            h = categories[gv];
            if h == nil then
                for category, ladders in pairs(categories) do
                    if ci(category, gv) then h = ladders; break; end
                end
            end
            if type(h) ~= 'table' then return nil, 'no gathering gear for this category'; end
        end
        local slotKey = string.lower(tostring(slot or ''));
...
-- HOW MANY OF THIS ITEM CAN I EQUIP RIGHT NOW (2026-08-03, Coffeepoo's field
-- report: two Bone Knives +1, the Main took one and the Sub was left as worn).
-- The same-name off-hand rule (utils.subSlotAllowed) needs proof of a SECOND
-- copy, and its only evidence at equip time was the `Count` stamp in gear.lua
-- -- which renderEntry writes once, at first index, and which no command can
-- refresh: /dl sync is add-only (an already-known item is skipped) and /dl fix
-- backfills CATALOG facts only, as it must, because "how many do you own" is
-- not one. Buy the second copy after the first was indexed and dlac could
-- never learn about it.
--
-- Ownership is a live fact and this state can just read it, exactly as the
-- Pair/RSlot ruling of 2026-08-01 said: read the fact, do not make the user
-- run a migration for it. bagCounts IS the scan (equip-eligible containers,
-- currently-worn pieces included -- they still occupy their wardrobe slot),
-- cached per second, so the flatten may ask per candidate.
--
-- nil means "the bags could not be read", NOT "you own none": the caller must
-- fall back to the file stamp rather than demote gear on a failed scan.
function M.bagCopies(id)
    id = tonumber(id);
    if id == nil then return nil; end
    local byId = bagCounts(false);
...
local CLAIMANTS = {
    ...
    { name = 'ModeLock',
      -- The QUEUE is captured beside the plan (2026-08-03): the losers of a
      -- contested slot have to reach the decision RING, not just the editor,
      -- because the Arbiter Monitor renders stashed records -- including pinned
      -- historical ones -- and deriving the queue live there would show TODAY's
      -- answer under a decision from ten minutes ago. Same law as the ladders
      -- and the reserve verdict: the queue that decided is the queue you read.
      -- Stashed on M rather than returned, because every other hook on this row
      -- indexes the ensure state as a plain slot -> entry map.
      ensure = function() return (M.modeLockLive()); end,
      ...
      sig = function(equip, on, st)
          ...
          table.sort(ks);
          -- THE QUEUE IS PART OF THE EXPLANATION, so it is part of the leg
          -- (2026-08-03) -- the rank-order leg's reasoning (v152) exactly:
          -- turning on a mode that queues behind the winner moves NO gear and
          -- no claim, so without this the trace and /dl why would keep saying
          -- "nobody is waiting" until something unrelated happened to move.
          -- It does NOT reach the decision ring, and must not: the ring appends
          -- on a moved OUTCOME, and a queue-only record with zero changed slots
          -- is the v163 symptom, not a feature.
          local qs = {};
          for _, q in ipairs(M._modeLockQ or {}) do
              qs[#qs + 1] = tostring(q.slot) .. '<' .. tostring(q.lost.by);
          end
          table.sort(qs);
          return 'ml:' .. table.concat(ks, ',') .. (qs[1] and ('|q:' .. table.concat(qs, ',')) or '');
      end,
      apply = function(env)
    ...
    pcall(function()
        ...
        local function buildContest()
            return { explain = M.arbExplain(claimsView(), arbOrder, floorTbl),
                     ...
                     src = slotSrc,
                     -- The mode-lock QUEUE as it stood when this dispatch
                     -- decided: who was waiting behind the mode that won the
                     -- slot. Travels with the record so a PINNED decision
                     -- explains itself rather than borrowing the present.
                     mlq = M._modeLockQ };
        end
