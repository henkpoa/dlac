        ...
        do
            ...
            for i = #arbOrder, 1, -1 do
                ...
                if cn == 'Locks' and next(M.locks) ~= nil then
                    -- Henrik's field refinement (2026-07-27 evening): a lock
                    -- on an EMPTY slot does not defend. Freezing "empty" is
                    -- exactly what a reservation preserves, so the reserver
                    -- may land (his case: a locked, empty Head must not stop
                    -- the Royal Cloak). A lock on a WORN piece defends as
                    -- before -- the reserver landing would make the server
                    -- strip the very piece the lock froze. The ceiling below
                    -- stays UNCONDITIONAL on purpose: free equip promises
                    -- "hand-equip any time and it stays", and an empty free
                    -- slot must keep that promise for the FUTURE hand-equip,
                    -- not just the current moment.
                    local held = nil;
                    for ls in pairs(M.locks) do
                        local canon = CANON_OF[ls] or ls;
                        if wornItemName(canon) ~= nil then
                            held = held or {};
                            held[canon] = '(locked)';
                        end
                    end
