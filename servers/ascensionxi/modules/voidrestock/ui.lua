...
local function button(label, enabled, action)
    if enabled then if imgui.Button(label) then action(); end
    else imgui.TextDisabled(esc(label)); end
end
...
local function drawList(scope, title, entries, counts)
    ...
    for _, entry in ipairs(entries) do
        ...
        imgui.PushID(key);
        imgui.Text(esc(string.format('%s   Inventory: %d   Void: %s', entry.name,
            counts[entry.id] or 0, client.fresh and tostring(client.counts[entry.id] or 0) or '?')));
        imgui.SetNextItemWidth(95); imgui.InputInt('Keep', edits[key]);
        imgui.SameLine();
        button('Save target', not restock.busy(), function()
            local config = copy();
...
function M.render()
    ...
    drawList('character', 'Always (every job)', restock.config.character, counts or {});
    drawList('job', restock.job() .. ' only', restock.config.jobs[restock.job()] or {}, counts or {});
    ...
    -- Exact resource names also allow planning before acquiring an item.
    if query[1] ~= '' then
        pcall(function()
            local r = AshitaCore:GetResourceManager():GetItemByName(query[1], 2)
                or AshitaCore:GetResourceManager():GetItemByName(query[1], 0);
    ...
    for id in pairs(candidates) do
        ...
        if query[1] == '' or rec.name:lower():find(query[1]:lower(), 1, true) then rows[#rows + 1] = rec; end
    end
    ...
    for i = 1, math.min(30, #rows) do
        ...
        imgui.Text(esc(rec.name)); imgui.SameLine();
        for _, scope in ipairs({ 'character', 'job' }) do
            ...
            if scope == 'character' then imgui.SameLine(); end
        end
        imgui.PopID();
    end
    if #rows > 30 then imgui.TextDisabled('Showing 30 matches; refine your search.'); end
...
local function trayButton(asset, fallback, tip, action)
    ...
    local clicked;
    if tex then clicked = imgui.ImageButton(tex, { 30, 30 });
...
function M.trayDraw()
    ...
    trayButton('ebox_red_icon-64', 'S', 'Store excess of listed items; keep inventory targets.', function()
        if ready then restock.start('store'); elseif not restock.busy() then client.refresh(); end
    end);
    if ready and #plan.fetch > 0 then
        trayButton('ebox', 'F', 'Fetch listed shortages from Void Storage.', function() restock.start('fetch'); end);
    end
end
return M;
