...
-- ---------------------------------------------------------------------------
-- THE YELLOW ICON's question (v2 grill C2, REVISED by Henrik 2026-07-28): which
-- tracked items are you short of out here, while copies of them sit at your MOG
-- HOUSE -- Mog Safe, Storage, Mog Locker, Mog Safe 2?
--
-- The original ruling counted the other FIELD bags (Satchel/Sack/Case) as
-- "somewhere you can't use it from" and offered to buy duplicates out of the
-- box. Field reality says otherwise: those bags are with you, one drag away, so
-- the icon was nagging about stock you already have. Mog House stock is the
-- real trap -- unreachable until you go home, and the box IS the only fix.
--
-- `held` is therefore everything you are CARRYING (Inventory + Satchel + Sack +
-- Case + what your quivers hold) -- the same on-hand the green plan uses -- so a
-- Mog Case copy silences this icon exactly the way it silences green.
--
-- ctx = { held(id) -> n on your person, stored(id) -> n in the Mog House }.
-- An entry qualifies when it has Mog House copies AND what you carry is below
-- target. Note this does NOT require the box to hold any: "the box can't help
-- and yours are at home" is the most useful thing this icon ever says. Pure (RS*).
-- ---------------------------------------------------------------------------
function M.homeStockNeed(entries, ctx)
    entries, ctx = entries or {}, ctx or {};
    local held   = (type(ctx.held)   == 'function') and ctx.held   or function() return 0; end;
    local stored = (type(ctx.stored) == 'function') and ctx.stored or function() return 0; end;
    local out = {};
    for _, e in ipairs(entries) do
        local target = math.max(0, math.floor(tonumber(e.target) or 0));
        local h = math.max(0, math.floor(tonumber(held(e.id))   or 0));
        local s = math.max(0, math.floor(tonumber(stored(e.id)) or 0));
        if s > 0 and h < target then
            out[#out + 1] = { id = e.id, name = e.name, target = target,
                              held = h, stored = s, want = target - h };
        end
    end
    return out;
end

function M.needsHomeStock(entries, ctx)
    return #M.homeStockNeed(entries, ctx) > 0;
end
