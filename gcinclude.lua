...
gcdisplay = gFunc.LoadFile('dlac\\gcdisplay.lua');

gcinclude.AliasList = T{'gcmessages','wsdistance','setcycle','dt','th','kite','meleeset','gcdrain','gcaspir','nukeset','burst','weapon','elecycle','helix','weather','nuke','death','fight','sir','tankset','proc','cj','pupmode','tpgun','cormsg','forcestring','siphon','warpring','provring', 'telering','rrset','craftset','fieldset','zeniset','fishset'};
gcinclude.Towns = T{'Tavnazian Safehold','Al Zahbi','Aht Urhgan Whitegate','Nashmau','Southern San d\'Oria [S]','Bastok Markets [S]','Windurst Waters [S]','San d\'Oria-Jeuno Airship','Bastok-Jeuno Airship','Windurst-Jeuno Airship','Kazham-Jeuno Airship','Southern San d\'Oria','Northern San d\'Oria','Port San d\'Oria','Chateau d\'Oraguille','Bastok Mines','Bastok Markets','Port Bastok','Metalworks','Windurst Waters','Windurst Walls','Port Windurst','Windurst Woods','Heavens Tower','Ru\'Lude Gardens','Upper Jeuno','Lower Jeuno','Port Jeuno','Rabao','Selbina','Mhaura','Kazham','Norg','Mog Garden','Celennia Memorial Library','Western Adoulin','Eastern Adoulin'};
gcinclude.LockingRings = T{'Empress Band', 'Echad Ring','Venture Ring', 'Trizek Ring', 'Endorsement Ring', 'Capacity Ring', 'Warp Ring', 'Venture Ring', 'Resolution Ring', 'Provenance Ring','Facility Ring','Dim. Ring (Dem)','Dim. Ring (Mea)','Dim. Ring (Holla)'};
...
function gcinclude.HandleCommands(args)
	...
	elseif (args[1] == 'provring') then
		gcinclude.DoProvRing();
	...
	elseif (args[1] == 'fieldset') then
		gcinclude.FieldSet = not gcinclude.FieldSet;
		toggle = 'Field Set';
		status = gcinclude.FieldSet;
...
function gcinclude.DoProvRing()
	AshitaCore:GetChatManager():QueueCommand(1, '/lac equip ring2 "Provenance Ring"');

	local function usering()
		local function forceidleset()
			AshitaCore:GetChatManager():QueueCommand(1, '/lac set Idle');
		end
		AshitaCore:GetChatManager():QueueCommand(1, '/item "Provenance Ring" <me>');
		forceidleset:once(8);
	end
...
function gcinclude.CheckDefault()
	...
	if (gcinclude.FieldSet == true) then gFunc.EquipSet(gcinclude.sets.Field) end
