...
FILES = [
    ...
    "sql/mob_spawn_points.sql",        # mobid -> name; idx = mobid & 0xFFF (widescan name resolution)
...
def load(refresh=False):
    ...
    # spawn points: mobid encodes zone+index (mobid = 0x1000000 + zone*4096 + idx,
    # verified against a live uniqueNo) -> per-zone widescan idx -> display name.
    # The widescan reply carries NO name on CatsEyeXI (field 2026-07-14); names
    # beyond render range can ONLY come from this map.
    db.spawn_idx = {}
    t = src["sql/mob_spawn_points.sql"]
    cols = sql_columns(t, "mob_spawn_points")
    if cols:
        mid_i, disp_i, name_i = cols.index("mobid"), cols.index("polutils_name"), cols.index("mobname")
        for r in sql_rows(t, "mob_spawn_points"):
            mobid = int(r[mid_i])
            zone, idx = (mobid - 0x1000000) >> 12, mobid & 0xFFF
            disp = r[disp_i] or r[name_i].replace("_", " ")
            db.spawn_idx.setdefault(zone, {})[idx] = disp
    else:
        db.warns.append("DRIFT: mob_spawn_points.sql columns not parsed")
...
def dump_luadata(db, path):
    ...
    with open(path, "w", encoding="utf-8", newline="\n") as f:
        ...
        f.write("},\nidx = {\n")
        ni = 0
        for zid in sorted(db.spawn_idx):
            if zid not in zones:
                continue                       # no mob data for the zone -> map is dead weight
            f.write("[%d]={" % zid)
            for i in sorted(db.spawn_idx[zid]):
